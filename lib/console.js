...
var regexPlaceholder = /:(\w+)/g;
function getPlaceholderReplacer(data) {
  return function (match, p1) {
    return (data && typeof data === 'object' && data[p1]) ? data[p1] : match;
  };
}
...
Console.prototype.access = function (module, data, tags) {
    ...
    line = line.replace(':tags', tagstring);
    line = line.replace(regexPlaceholder, getPlaceholderReplacer(data));
    console.log(line);
};
...
Console.prototype.exception = function (time, module, err, tags) {
    ...
    line = line.replace(':tags', tagstring);
    line = line.replace(regexPlaceholder, getPlaceholderReplacer(err));
    console.error(line);
};
